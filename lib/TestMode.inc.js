						...
						,	desc // = ''
							...
							for(; !folC.atEnd(); folC.moveNext() ) {
								...
								for( var k in this.simple ){
									...
									if( !$$.fso.FileExists( $$.fso.BuildPath( fItem.Path, this.objects[k].executableName ) ) ) continue; 
...
,	'_getCompiler':		function( 
							...
							match = ( str.match( /\bVC[0-9]\b/i ) || [] ).shift().toUpperCase();
...
var TestModeCLI = Class.create( TestMode, {
	...
	'getCLI':		function( 
						...
						var command = $$.fso.BuildPath( this.phpExePath, 'php.exe' ) +this._ini+' '+pathToFile 
	...
	'_applyIni':	function(
						...
						// Looks tricky, but really just turn the ini
						// into a bunch of: -d "directive=value"
						this._ini = ( ini ? ' -d "{0}"'.Format(ini.toString('" -d "')): '');
					}
});
...
TMSB.registerConstant( 'cli', PHP.VC6|PHP.VC9, PHP.ZTS|PHP.NTS, 'php.exe', TestModeCLI );
