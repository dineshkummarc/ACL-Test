var TestCase = Class.create( TestBenchFactor, {
	 ...
	 */
	'describe':		function(){
						return getRelativePath( 'phpt', this.phptPath ).replace(/\.(.+)$/,'');
					},
	/**
	...
	'save':			function( basePath, extension ) {
						...
						var fullPath = $$.fso.BuildPath( 
								...
								this.phptPath.split( '\\' ).pop().split( '.' ).slice( 0, -1 ).splice( 0,0, extension ).join( '.' )
							)
						,	a = WL(basePath+'\n'+fullPath)
						,	out = $$.fso.CreateTextFile( fullPath )
						;
						WL(fullPath);
						for( var sectionName in this.phptParts ) { WL(sectionName);
