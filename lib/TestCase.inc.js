var TestCase = Class.create( TestBenchFactor, {
	...
	'parse':		function(){ 
						...
						// Here we do some magic to split the lines while keeping line endings in tact. 
						// The TextStream object has a ReadLine() method that strips line endings, so we don't use it
						// Instead, we REVERSE the string, iterate through to find the next ending, and make sure it is
						// included in the output before REVERSING it back.
...
,	
	 ...
	 * @return {String}
	 */	
	'stripIllegalWhitespace':function( str ){ return str;
...
var TestCaseStackLoader = Class.create(TestCaseStack,{
	/**
	 * Start up the autoloader.
	 */
	initialize: function( $super, path, positivePattern, negativePattern ){ //try{
		$super();
		this._loadFromPath.apply( this, [ path, positivePattern, negativePattern] );
	},
	 ...
	 */
	_loadFromPath: function( path, positivePattern, negativePattern ){
		var path = ( path || '.\\' ).Format()
		,	posi = positivePattern || /\.phpt$/
		,	negi = negativePattern || null
		;
		...
		for(; !fC.atEnd(); fC.moveNext() ) {
			var tF = fC.item();
			if( tF.Name.match( posi ) && 
				( typeof( negi )=='undefined' || !tF.Path.match( negi ) ) 
			) {
		...
		for(; !fS.atEnd(); fS.moveNext() ) {
			this._loadFromPath( fS.item().Path, posi, negi );
		}
		return this;
	}
});
TestCaseStackLoader.Lazy = Class.createLazyLoader(TestCaseStackLoader);