...
TestContext.Extend( ( function(){
	...
	,	testContextTwo
		...
		setupContexts['A']=testContextOne.getDir( 'A' );
		_populate( options, setupContexts['A'] );
		
		if( !link.length ) return setupContexts;
		
		testContextOne.setup( 'B' );
		setupContexts['B']=testContextOne.getDir( 'B' );
		_populate( options, setupContexts['B'] );
		
		availableLinks = ['symbolic','junction','hard'];
		for( var linkType in availableLinks ) {
			if( linkAll || Array.valueExists( link, linkType ) ) { 
				_linkToAll( 
					linkType
					, $$.fso.BuildPath( setupContexts['B'], linkType ) 
					, setupContexts['A']
				);
				_linkToAll( 
					linkType
					, $$.fso.BuildPath( setupContexts['A'], linkType ) 
					, setupContexts['B']
				);
			}
		}
	...
	,	testContextTwo
		...
		,	destroyedContexts = {}
		; 
		if( !options.filesystem || !pop.length ) return destroyedContexts;
		
		testContextOne.destroyDir( 'A' );
		destroyedContexts['A']=true;
		
		if( !link.length ) return destroyedContexts;
	...
	function _tryLinkFolder( type, from, to ) {
		switch( type ) {
			case 'symbolic':
				return $$( 'MKLINK /D {0} {1}', from, to );
			case 'junction':
				return $$( 'MKLINK /J {0} {1}', from, to );
			case 'hard':
				// ensure that they have the same root before making this one work
			default:
				return false;
	...
	function _formatIndividualOption( option ){
		return (
			...
			( pop === false || Object.isUndefined( pop ) ) ? [] : // boolean/unset false means none
