...
,	_tokenizeLine: function PhpConfig__tokenizeLine( line ) {
		...
		if( ( command = line.split(';').slice(0,1).join() ) && 
			...
			if( (eqPos = line.indexOf('=')) >= 0 ){
				token[2] = line.slice(0,eqPos).trim(); // key
				token[3] = line.slice(eqPos+1).trim().unquote(); // raw value
...
,	toString: function PhpConfig_toString(seperator){
		...
		for( var i = 0; i < this.tokens.length; i++ ) {
			...
			// In some cases we need to quote the value.
			if( this.tokens[i][0] != this.COMMENT && this.tokens[i][3].match(/^[&]+$/) ) {
				ret.push( [this.tokens[i][2], this.tokens[i][3].quote()].join('=') );
				continue;
			}
