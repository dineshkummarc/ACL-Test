...
$$.Extend( {
    ...
    folderStack:[],
	cmd:function(){ 
		// Call in CMD sub-process ( Wrap in CMD /C )
...
var Assert = {
    ...
    IsAdmin: function() {
        if(typeof($SESSIONNAME)!='undefined') // NON-ADMIN CONSOLES USE THIS ENV VARIABLE
	    return Assert.Fail('Shell requires Admin privileges.');
    },
    Fail: function() {
...
function folderIsEmpty(folder) {
	...
	for(; !fc.atEnd();fc.moveNext())
		contents++;
	var fc = new Enumerator(fol.SubFolders);
	for(; !fc.atEnd();fc.moveNext())
		contents++;
	return (!contents);
}
...
function mkdirRecursive(folder){
	...
	if(!folderExists($$.fso.GetParentFolderName(folder))){
		mkdirRecursive($$.fso.GetParentFolderName(folder));
	}
	mkdir(folder);
}
...
//BEGIN: insertion by Ryan Biesemeyer
function getRelativePath( fromDir, toFileOrDir ) {
	...
	for(var depth in fromDir) {
		if(fromDir[depth]==toFileOrDir[depth])
			commonDepth++;
		else
			break;
	}
	// find how many levels we have to go back in from "to" get to the common dir
	var r = ( new Array( fromDir.length - commonDepth + 1 ) ).join( '..\\' );
	
	// append what's not common of the "to";
	r += toFileOrDir.slice(commonDepth).join('\\');
	
	return r;
}
// END: insertion by Ryan  Biesemeyer
