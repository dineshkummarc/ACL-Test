...
var TestResult = Class.create({
	...
	'getSavePath': 	function(){
						for( var factorType in this.factors ){
							if(this.factors.hasOwnProperty(factorType))
								$Globals[factorType] = this.factors[factorType].describe();
						}
						var logDir = config.results.pathTemplate.Format();
						return logDir;
					},
	'save':			function(){
						var logDir = this.getSavePath();
						...
						stream.WriteLine( this.factors.testCase.phptPath );
						stream.Close();
					}
});
...
var TestResultFail = Class.create( TestResult, {
	 ...
	 */
	'initialize':	function( testCase, actual ) {
