var TestBench = Class.create({
	...
	'run':			function TestBenchºrun( testModeOrTestModeStack ) {
						...
						LOG('test modes:           '+testModes.length(), 			LOG.PREFLIGHT_SUMMARY );
						LOG('test cases:           '+this.testCases.length(), 		LOG.PREFLIGHT_SUMMARY );
						if( this.testContextsA )
						LOG('test contexts A:      '+this.testContextsA.length(), 	LOG.PREFLIGHT_SUMMARY );
						
						if( this.testContextsB )
						LOG('test contexts B:      '+this.testContextsB.length(), 	LOG.PREFLIGHT_SUMMARY );
						
						LOG('---------------------------------------', 					LOG.PREFLIGHT_SUMMARY );
						LOG('total test scenarios: '+(	testModes.length() * 
														this.testCases.length() * 
														( this.testContextsA.length() || 1 )* 
														( this.testContextsB.length() || 1 ) 
