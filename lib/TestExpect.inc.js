var TestExpect = Class.create( {
		...
		'getDiff':function getDiff(){
			...
			this._diff = this._reduceNoise( this._getDiff( A, B ) );
		...
		'_diffEngine':function _diffEngine( A, B ) {
			...
			if( ( A.length + B.length ) < 20 ) { //WL('_diffMap:'+A.length+':'+B.length);
				...
				WL('cutting a chunk off: '+A.length+'/'+B.length);
				return [].concat(
					this._getDiff( A.slice( 0, 6 ), B.slice( 0, 6 ) ),
					this._getDiff( A.slice( 6 ), B.slice( 6 ) )
				);
			}
